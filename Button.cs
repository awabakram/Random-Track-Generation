...
namespace Random_Track_Generation
{
    class Button
    {
        ...
        public Button(int newHeight, int newWidth, string text, Vector2 position, SpriteFont newfont, Color newColor)
        {
            height = newHeight;
            width = newWidth;
            buttonText = text;
            ...
            stringPos = new Vector2(buttonPosition.X + (width / 2), buttonPosition.Y + (height / 2));
        ...
        {
            spriteBatch.DrawRectangle(buttonPosition, new Size2(width, height), currentButtonColor, 10);
